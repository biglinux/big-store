...
# Install Snap
if [ -n "$PKG_SNAP_INSTALL" ]; then       
    SNAP_INFO="$(grep $PKG_SNAP_INSTALL $HOME/.bigstore/snap.cache | rev | cut -f2 -d '|' | rev)"
    snap install $SNAP_INFO
fi

# Remove Snap
if [ -n "$PKG_SNAP_REMOVE" ]; then 
    SNAP_INFO="$(grep $PKG_SNAP_REMOVE $HOME/.bigstore/snap.cache | rev | cut -f2 -d '|' | rev)"
    snap remove $SNAP_INFO
fi
