...
def print_pkg_details (details):
    ...
    if details.get_installed_version() is None:
        with open(os.path.expanduser('/tmp/big-select.tmp')) as e:
            checkedBoxItem = 'checked' if details.get_name() + ',install,native' in e.read() else ''
        print ('<div id=appstream_not_installed>'+_('Instalar')+'</div></a></div>')
        print ('<form id=formcheckbox><div id=checkboxItem><input type=checkbox id=itemSelect-' + details.get_name() + ' name=itemSelect class=checkboxitemSelect-native value=' + details.get_name() + ',install,native '+ checkedBoxItem +'><label for=itemSelect-' + details.get_name() + '></label></div></form>')
        print ('</div>')        
        ...
        with open('/tmp/bigstore/upgradeable.txt') as f:
            if '\n' + details.get_name() + '\n' in f.read():
                with open(os.path.expanduser('/tmp/big-select.tmp')) as e:
                    checkedBoxItem = 'checked' if details.get_name() + ',remove,native' in e.read() else ''
                print ('<div id=appstream_upgradable>'+_('Atualizar')+'</div></a></div>')
                print ('<form id=formcheckbox><div id=checkboxItem><input type=checkbox id=itemSelect-' + details.get_name() + ' name=itemSelect class=checkboxitemSelect-native value=' + details.get_name() + ',remove,native '+ checkedBoxItem +'><label for=itemSelect-' + details.get_name() + '></label></div></form>')
                print ('</div>')                
            else:
                with open(os.path.expanduser('/tmp/big-select.tmp')) as e:
                    checkedBoxItem = 'checked' if details.get_name() + ',remove,native' in e.read() else ''
