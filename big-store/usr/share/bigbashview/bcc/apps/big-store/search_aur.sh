...
IFS=$'\n'
#for line in $(LC_ALL=C yay -Ssa $* --sortby popularity); do
...
while read -r line; do
	if [[ $line == aur/* ]]; then
		...
		#		echo "<a onclick=\"disableBody();\" href=\"view_aur.sh.htm?pkg_name=$pkg\">" >>"$TMP_FOLDER/aur_build.html"
		#		echo "<div class=\"col s12 m6 l3\" id=$aur_priority>" >>"$TMP_FOLDER/aur_build.html"
		#		echo "<div class=\"showapp\">" >>"$TMP_FOLDER/aur_build.html"
		#		echo "<div id=aur_icon><div class=icon_middle>$icon</div>" >>"$TMP_FOLDER/aur_build.html"
		#		echo "<div id=aur_name><div id=limit_title_name>$title_uppercase_first_letter</div>" >>"$TMP_FOLDER/aur_build.html"
		#		echo "<div id=version>$version</div></div></div>" >>"${TMP_FOLDER}/aur_build.html"
		#		echo "<div id=box_aur_desc><div id=aur_desc>$summary</div></div>" >>"$TMP_FOLDER/aur_build.html"
		#		echo "$button</a></div></div>" >>"$TMP_FOLDER/aur_build.html"
