...
def print_pkg_details (details):
    ...
    if details.get_installed_version():
        ...
        with open(os.path.expanduser('/tmp/big-select.tmp')) as e:
            ...
            lineCheck = sys.argv[1] + ',remove,native,' + iconCheck.strip() + ','+ details.get_id() +','+ details.get_version()
            
            checkedBoxItem = 'checked' if lineCheck in e.read() else ''
            
            print ('<form id=formcheckbox><div><input type=checkbox id=itemSelect-' + sys.argv[1] + ' name=itemSelect class=checkboxitemSelect-native value="'+ lineCheck +'" '+ checkedBoxItem +'><label for=itemSelect-' + sys.argv[1] + '>', _('Adicionar na lista'), '</label></div></form>')
        ...
        with open('/tmp/bigstore/upgradeable.txt') as f:
            ...
            with open(os.path.expanduser('/tmp/big-select.tmp')) as e:
                ...
                lineCheck = sys.argv[1] + ',install,native,' + iconCheck.strip() + ','+ details.get_id() +','+ details.get_version()
